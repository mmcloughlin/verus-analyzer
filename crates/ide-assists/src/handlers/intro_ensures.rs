...
use syntax::{
    ast::{
        self,
        make::{self, assert_stmt_from_predicate, expr_path, ext::ident_path, let_stmt},
    },
    AstNode,
};
...
pub(crate) fn intro_ensures(acc: &mut Assists, ctx: &AssistContext<'_>) -> Option<()> {
    ...
    acc.add(
        ...
        "Copy ensures clauses to the end",
        ensures_range,
        |edit| {
        ...
        Some(ret_expr) => {
            let (ret_name, new_tail_expr) = match func.ret_type() {
                Some(ret_t) => {
                    ...
                    (ret_name.clone(), expr_path(ident_path(&format!("{ret_name}").as_str())))
                    // REVIEW: asssume the returning value is typed if it is returning something
                }
                ...
                None => (Pat::WildcardPat(make::wildcard_pat()), make::expr_unit()),
            };

            // add let binding of return value
            let let_stmt =
                let_stmt(ret_name.clone(), None, Some(ret_expr.clone())).clone_for_update();
            ...
            ted::insert(
                ted::Position::first_child_of(first_assert_stmt.syntax()),
            ...
            while let Some(ens) = ensures_clauses.next() {
                ...
                let assert_stmt_without_indent =
                    assert_stmt_from_predicate(ens_without_comma).clone_for_update();
                ted::insert(
                    ted::Position::first_child_of(assert_stmt_without_indent.syntax()),
                    ast::make::tokens::whitespace(&format!("\n{}", indent_level)),
                );
            ...
            ted::insert(
                ted::Position::before(stmt_list.tail_expr()?.syntax()),
                ast::make::tokens::whitespace(&format!("\n{}", indent_level)),
            );
            return Some(func);
        }
