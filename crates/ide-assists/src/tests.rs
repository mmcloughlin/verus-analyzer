...
fn check(handler: Handler, before: &str, expected: ExpectedResult<'_>, assist_label: Option<&str>) {
    ...
    let verus_path: String =
        "/Users/chanhee/Works/secure-foundations/verus/source/tools/rust-verify.sh".to_string();
    ...
    let ctx = AssistContext::new(sema, &config, frange, vec![], vec![]); //TODO(verus)
